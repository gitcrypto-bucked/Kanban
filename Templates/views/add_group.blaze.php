    ...
    <section class="p-6 max-w-3xl">
      ...
      <div class="bg-slate-950 border border-slate-800 rounded-lg p-6 space-y-6">
        ...
        <div>
          ...
          <input id="groupName" type="text" class="w-full bg-slate-800 border border-slate-700 rounded px-3 py-2 focus:outline-none focus:ring-2 focus:ring-accent" placeholder="Desenvolvedores" />
        </div>

        <div>
          <label class="block mb-2 text-sm">Descrição</label>
          <textarea id="groupDesc" class="w-full bg-slate-800 border border-slate-700 rounded px-3 py-2 focus:outline-none focus:ring-2 focus:ring-accent" placeholder="Descrição do grupo"></textarea>
        </div>

        <!-- Users -->
        <div>
          <label class="block mb-2 text-sm">Usuários</label>
...
function createGroup() {
  ...
  const group = {
    ...
    form.append('users',selectedUsers);
