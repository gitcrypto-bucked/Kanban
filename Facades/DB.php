...
class DB
{
    ...
    {
        $this->whereOr[] = ['column' => $column, 'operator' => $operator, 'value' => $value];
        $this->params[":{$column}"] = $value; // Simple parameter binding example
        return $this;
    }
    ...
    public function get()
    {
        ...
        if (!empty($this->whereOr) && !empty($this->where)) 
        {
            ...
            $this->sql .= " OR " . implode(' OR ', $whereClauses);
        }
        ...
        if (!empty($this->whereOr) && empty($this->where)) 
        {
            ...
            $this->sql .= " WHERE " . implode(' OR ', $whereClauses);
        }
    ...
    public function insert_last_ID(array $data)
    {
        $SQL = "INSERT INTO {$this->table} (".implode(', ', array_keys($data)).") VALUES ( :".implode(', :',array_keys($data)).")";
        $conn = Datatables::getInstance()->getConnection();
